---
name: obsidian-bases
description: 创建和编辑 Obsidian Bases（.base 文件），包含视图、过滤器、公式和摘要。当用户使用 .base 文件、创建类似数据库的笔记视图，或提到 Bases、表格视图、卡片视图、过滤器或公式时使用。
---
...
5. **验证**：验证文件是有效的 YAML，没有语法错误。检查所有引用的属性和公式是否存在。常见问题：包含特殊 YAML 字符的未加引号字符串、公式表达式中的引号不匹配、在 `formulas` 中未定义 `X` 时引用 `formula.X`
6. **在 Obsidian 中测试**：在 Obsidian 中打开 `.base` 文件以确认视图正确渲染。如果显示 YAML 错误，请检查下面的引号规则
...
1. **笔记属性** - 来自前置内容：`note.author` 或仅 `author`
...
| `file.properties` | Object | 所有前置内容属性  |
...
最常用的函数。有关所有类型（Date、String、Number、List、File、Link、Object、RegExp）的完整参考，请参见 [FUNCTIONS\_REFERENCE.md](references/FUNCTIONS_REFERENCE.md)。
...
**重要：** Duration 不直接支持 `.round()`、`.floor()`、`.ceil()`。首先访问数字字段（如 `.days`），然后应用数字函数。
...
- 对于包含双引号的公式使用单引号：`'if(done, "Yes", "No")'`
- 对于简单字符串使用双引号：`"我的视图名称"`
- 在复杂表达式中正确转义嵌套引号
...
**公式中引号不匹配**：当公式包含双引号时，将整个公式用单引号包裹。
...
**不访问字段的 Duration 数学**：减去日期返回 Duration，不是数字。始终访问 `.days`、`.hours` 等。
...
**引用未定义的公式**：确保 `order` 或 `properties` 中的每个 `formula.X` 在 `formulas` 中都有对应的条目。
